...
struct outage_data
    ...
    outage_data(
        prime_mover = "PrimeMovers.Default",
        thermal_fuel = "ThermalFuels.Default",
        capacity = 100,
        FOR = 0.5,
        MTTR = 50,
    ) = new(prime_mover, thermal_fuel, capacity, FOR, MTTR)
end
...
function get_available_components_in_aggregation_topology(
    type::Type{<:PowerSystems.StaticInjection},
    sys::PSY.System,
    region::PSY.AggregationTopology,
)
    avail_comps = [
        comp for
        comp in PSY.get_components_in_aggregation_topology(type, sys, region) if
...
function make_pras_system(sys::PSY.System;
    ...
    #######################################################
    dup_uuids = [];
    h_s_comps = if availability_flag
        PSY.get_components(x -> PSY.get_available(x) == true, PSY.HybridSystem, sys)
    else
    ...
    sys_for_int_in_hour =
        round(Dates.Millisecond(PSY.get_forecast_interval(sys)), Dates.Hour)
    sys_res_in_hour =
        round(Dates.Millisecond(PSY.get_time_series_resolutions(sys)[1]), Dates.Hour)
    ...
    start_datetime =
        start_datetime + Dates.Hour((period_of_interest.start-1)*sys_res_in_hour);
    ...
    regions = sort!(
        collect(PSY.get_components(aggregation_topology, sys));
        by = x -> lowercase(x.name),
    );
    if (length(regions)!=0)
        @info "The PSY System has $(length(regions)) regions based on PSY AggregationTopology : $(aggregation_topology)."
    else
        error(
            "No regions in the PSY System. Cannot proceed with the process of making a PRAS SystemModel.",
        )
    end
    ...
    for (idx, region) in enumerate(regions)
        reg_load_comps = if availability_flag
            get_available_components_in_aggregation_topology(PSY.StandardLoad, sys, region)
        else
    ...
    if (lump_pv_wind_gens)
        for (idx, region) in enumerate(regions)
            reg_ren_comps = if availability_flag
            ...
            reg_gen_comps = if availability_flag
            ...
            gs = [
                g for g in reg_gen_comps if (
                    typeof(g) != PSY.HydroTurbine && PSY.get_max_active_power(g)!=0 &&
                    PSY.IS.get_uuid(g) ∉ union(
                        dup_uuids,
                        PSY.IS.get_uuid.(wind_gs_DA),
                        PSY.IS.get_uuid.(pv_gs_DA),
                    )
                )
            ]
            push!(gens, gs)
            ...
            if (idx==1)
                ...
                if (
                    length(reg_wind_gens_DA[idx - 1]) > 0 &&
                    length(reg_pv_gens_DA[idx - 1]) > 0
                )
    ...
    for (idx, region) in enumerate(regions)
        ...
        reg_stor_comps = if availability_flag
    ...
    for (idx, region) in enumerate(regions)
        reg_gen_stor_comps = if availability_flag
            get_available_components_in_aggregation_topology(PSY.StaticInjection, sys, region)
        else
            PSY.get_components_in_aggregation_topology(PSY.StaticInjection, sys, region)
        end
        gs = [
            g for g in reg_gen_stor_comps if
            (typeof(g) == PSY.HydroTurbine || typeof(g)==PSY.HybridSystem)
        ]
        push!(gen_stors, gs)
        if idx==1
            start_id[idx] = 1
    ...
    for (idx, g) in enumerate(gen)
        ...
        if (
            ...
            if (
                ...
                ("max_active_power" in PSY.get_name.(PSY.get_time_series_multiple(g)))
            )
                gen_cap_array[idx, :] = floor.(
                    Int,
                    get_forecast_values(
                        first(PSY.get_time_series_multiple(g; name = "max_active_power")),
                    )
                    *
                ...
                gen_cap_array[idx, :] =
                    fill.(floor.(Int, PSY.get_max_active_power(g)), 1, N);
            end
        end
        ...
        if (outage_ts_flag)
            try
                ...
                g_λ_μ_ts_data = outage_to_rate.(
                    zip(
                        outage_ts_data[!, PSY.get_name(g)],
                        fill(24, length(outage_ts_data[!, PSY.get_name(g)])),
                    ),
                )
            ...
            if (~outage_flag)
                if (gen_categories[idx] == "ThermalStandard")
                    ...
                    fl_idx = findall(
                        x -> x == fl,
                        getfield.(outage_values[p_m_idx], :thermal_fuel),
                    )
                    ...
                    if (length(temp_range)>1)
                        ...
                        for (x, y) in
                            zip(temp_range, getfield.(outage_values[temp_range], :capacity))
                            temp=0
                            if (temp<temp_cap<y)
                                gen_idx = x
                                break
                ...
                elseif (gen_categories[idx] == "HydroDispatch")
                    ...
                    if (length(p_m_idx)>1)
                        for (x, y) in
                            zip(p_m_idx, getfield.(outage_values[p_m_idx], :capacity))
                            temp=0
                            if (temp<temp_cap<y)
                                gen_idx = x
                ...
                if (
                    !(
                        haskey(ext, "outage_probability") &&
                        haskey(ext, "recovery_probability")
                    )
                )
    ...
    new_generators = PRAS.Generators{N, 1, PRAS.Hour, PRAS.MW}(
        gen_names,
    ...
    for (idx, s) in enumerate(stor)
        stor_charge_cap_array[idx, :] =
            fill(floor(Int, getfield(PSY.get_input_active_power_limits(s), :max)), 1, N);
        stor_discharge_cap_array[idx, :] =
            fill(floor(Int, getfield(PSY.get_output_active_power_limits(s), :max)), 1, N);
        stor_energy_cap_array[idx, :] =
            fill(floor(Int, getfield(PSY.get_storage_level_limits(s), :max)), 1, N);
    ...
    new_storage = PRAS.Storages{N, 1, PRAS.Hour, PRAS.MW, PRAS.MWh}(stor_names,
        ...
        if (typeof(g_s) == PSY.HydroTurbine)
            if (PSY.has_time_series(g_s))
                if ("inflow" in PSY.get_name.(PSY.get_time_series_multiple(g_s)))
                    gen_stor_charge_cap_array[idx, :] = floor.(
                        Int,
                        get_forecast_values(
                            first(PSY.get_time_series_multiple(g_s; name = "inflow")),
                        )
                        *
                        PSY.get_inflow(g_s),
                    );
                    gen_stor_discharge_cap_array[idx, :] = floor.(
                        Int,
                        get_forecast_values(
                            first(PSY.get_time_series_multiple(g_s; name = "inflow")),
                        )
                    ...
                    gen_stor_inflow_array[idx, :] = floor.(
                        Int,
                        get_forecast_values(
                            first(PSY.get_time_series_multiple(g_s; name = "inflow")),
                        )
                ...
                if ("storage_capacity" in PSY.get_name.(PSY.get_time_series_multiple(g_s)))
                    gen_stor_enrgy_cap_array[idx, :] = floor.(
                        Int,
                        get_forecast_values(
                            first(
                                PSY.get_time_series_multiple(
                    ...
                    gen_stor_enrgy_cap_array[idx, :] =
                        fill.(floor.(Int, PSY.get_storage_capacity(g_s)), 1, N);
                end
                if ("max_active_power" in PSY.get_name.(PSY.get_time_series_multiple(g_s)))
                    gen_stor_gridinj_cap_array[idx, :] = floor.(
                        Int,
                        get_forecast_values(
                            first(
                                PSY.get_time_series_multiple(
                    ...
                    gen_stor_gridinj_cap_array[idx, :] =
                        fill.(floor.(Int, PSY.get_max_active_power(g_s)), 1, N);
                end
            else
                gen_stor_charge_cap_array[idx, :] =
                    fill.(floor.(Int, PSY.get_inflow(g_s)), 1, N);
            ...
            gen_stor_charge_cap_array[idx, :] = fill.(
                floor.(
                    Int,
                    getfield(PSY.get_input_active_power_limits(PSY.get_storage(g_s)), :max),
                ),
                1,
                N,
            );
            gen_stor_discharge_cap_array[idx, :] = fill.(
                floor.(
                    Int,
                    getfield(
            ...
            gen_stor_enrgy_cap_array[idx, :] = fill.(
                floor.(
                    Int,
                    getfield(PSY.get_storage_level_limits(PSY.get_storage(g_s)), :max),
                ),
                1,
                N,
            );
            gen_stor_gridinj_cap_array[idx, :] = fill.(
                floor.(Int, PSY.getfield(PSY.get_output_active_power_limits(g_s), :max)),
                1,
                N,
            );
            ...
            if (
                PSY.has_time_series(PSY.get_renewable_unit(g_s)) && (
                    "max_active_power" in PSY.get_name.(
                        PSY.get_time_series_multiple(PSY.get_renewable_unit(g_s)),
                    )
                )
            )
                gen_stor_inflow_array[idx, :] = floor.(
                ...
                gen_stor_inflow_array[idx, :] = fill.(
    ...
    new_gen_stors =
        PRAS.GeneratorStorages{N, 1, PRAS.Hour, PRAS.MW, PRAS.MWh}(gen_stor_names,
            get_generator_category.(gen_stor),
            gen_stor_charge_cap_array, gen_stor_discharge_cap_array,
    ...
    if (system_model=="Zonal")
        ...
        for i in 1:num_interfaces
            ...
            if i==1
                ...
                start_id[i] = start_id[i - 1]+interface_dict[temp_regions_tuple[i - 1]][2]
            end
            interface_line_idxs[i] =
                range(start_id[i]; length = interface_dict[temp_regions_tuple[i]][2])
        end
        ...
        for i in 1:n_lines
            line_forward_capacity_array[i, :] = fill.(
                floor.(
                    Int,
                    getfield(line_rating(sorted_regional_lines[i]), :forward_capacity),
                ),
                1,
                N,
            );
            line_backward_capacity_array[i, :] = fill.(
                floor.(
                    Int,
                    getfield(line_rating(sorted_regional_lines[i]), :backward_capacity),
                ),
                1,
                N,
            );
        ...
        new_lines = PRAS.Lines{N, 1, PRAS.Hour, PRAS.MW}(
            line_names,
            line_categories,
            line_forward_capacity_array,
            line_backward_capacity_array,
            λ_lines,
            μ_lines,
        );
        #######################################################
        ...
        interface_regions_from = [
            findfirst(x->x==temp_regions_tuple[i][1], region_names) for
            i in 1:num_interfaces
        ];
        interface_regions_to = [
            findfirst(x->x==temp_regions_tuple[i][2], region_names) for
            i in 1:num_interfaces
        ];
        ...
        for i in 1:num_interfaces
            interface_forward_capacity_array[i, :] =
                sum(line_forward_capacity_array[interface_line_idxs[i], :]; dims = 1)
            interface_backward_capacity_array[i, :] =
                sum(line_backward_capacity_array[interface_line_idxs[i], :]; dims = 1)
        end
        ...
        pras_system =
            PRAS.SystemModel(new_regions, new_interfaces, new_generators, region_gen_idxs,
                new_storage, region_stor_idxs, new_gen_stors,
    ...
    elseif (system_model == "Single-Node")
        ...
        error(
            "Unrecognized SystemModel; Please specify correctly if SystemModel is Single-Node or Zonal.",
        )
    end
...
function generate_csv_outage_profile(pras_system::PRAS.SystemModel;
    location::Union{Nothing, String} = nothing, num_runs::Union{Nothing, Int} = nothing,
    ...
    shortfall_samples, gens_avail, stors_avail, gen_stors_avail, lines_avail = PRAS.assess(
        pras_system,
        PRAS.SequentialMonteCarlo(; samples = num_runs),
...
#######################################################
function add_csv_time_series!(sys_DA::PSY.System, sys_RT::PSY.System,
    outage_csv_location::String; days_of_interest::Union{Nothing, UnitRange} = nothing,
