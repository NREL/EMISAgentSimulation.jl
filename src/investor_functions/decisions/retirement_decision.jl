...
"""
function retire_project!(
    project::P,
) where {P <: Union{Project{Existing}, Project{Planned}, Project{Queue}}}
...
function take_retirement_decision(project::P,
    ...
    timeseries_data_dir::String) where {P <: Project{<: BuildPhase}, R <: RiskPreference}
    return
end
...
function take_retirement_decision(project::P,
    ...
    retirement_lookback::Int64,
    solver::JuMP.MOI.OptimizerWithAttributes,
    timeseries_data_dir::String) where {P <: Project{Existing}, R <: RiskPreference}
    finance_data = get_finance_data(project)
    ...
    if (get_construction_year(project) <= simulation_years)
        ...
        for scenario in scenario_data
            annual_revenue +=
                get_probability(scenario) *
                sum(
                    get_scenario_profit(finance_data)[get_name(scenario)][iteration_year][
                        :,
                        iteration_year,
                    ],
                )
        ...
        if (iteration_year > retirement_lookback)
            previous_cashflow =
                cash_flow[(iteration_year - retirement_lookback):(iteration_year - 1)]
            if (annual_profit < -1) && (project_utility < 0) &&
               (all(previous_cashflow .< -1))
                println("RETIRING:")
...
function take_retirement_decision(project::P,
    ...
    retirement_lookback::Int64,
    solver::JuMP.MOI.OptimizerWithAttributes,
    timeseries_data_dir::String) where {P <: Project{Planned}, R <: RiskPreference}
    finance_data = get_finance_data(project)
    queue_cost = get_queue_cost(finance_data)
...
function take_retirement_decision(project::P,
    ...
    retirement_lookback::Int64,
    solver::JuMP.MOI.OptimizerWithAttributes,
    timeseries_data_dir::String) where {P <: Project{Queue}, R <: RiskPreference}
    finance_data = get_finance_data(project)
    queue_cost = get_queue_cost(finance_data)
