...
function start_construction!(projects::Vector{<: Project{<: BuildPhase}},
    ...
    if iteration_year <= get_decision_year(project) + queue_time <=
       iteration_year + step_size - 1
    ...
    if iteration_year <= get_construction_year(project) <= iteration_year + step_size - 1
        ...
        if typeof(project) == RenewableGenEMIS{Planned}
            ...
            for scenario in scenario_names
                availability_df_rt = slice_year(
                    availability_rt_by_scenario[scenario],
                    iteration_year,
                    simulation_years,
                )
                availability_df = slice_year(
                    availability_by_scenario[scenario],
                    iteration_year,
                    simulation_years,
                )
                ...
                for year in 1:simulation_years
                    ...
                    load_n_vg_df[:, get_name(project)] =
                        availability_raw * get_maxcap(project)
                    ...
                    load_n_vg_df_rt[:, get_name(project)] =
                        availability_raw_rt * get_maxcap(project)
...
function finish_construction!(projects::Vector{<: Project{<: BuildPhase}},
    ...
    if iteration_year <= get_construction_year(project) <= iteration_year + step_size - 1
        ...
        for y in iteration_year:simulation_years
            for product in get_products(project)
                add_device_services!(
                    sys_MDs[y],
                    PSY.get_component(
                        typeof(PSY_project_MD),
                        sys_MDs[y],
                        PSY.get_name(PSY_project_MD),
                    ),
                    product,
                )
                add_device_services!(
                    sys_UCs[y],
                    PSY.get_component(
                        typeof(PSY_project_UC),
                        sys_UCs[y],
                        PSY.get_name(PSY_project_UC),
                    ),
                    product,
                )
                add_device_services!(
                    sys_EDs[y],
                    PSY.get_component(
                        typeof(PSY_project_ED),
                        sys_EDs[y],
                        PSY.get_name(PSY_project_ED),
                    ),
                    product,
                )
            end
        ...
        for scenario in keys(sys_PRAS)
            ...
            for product in get_products(project)
                ...
                add_device_services!(
                    sys_PRAS[scenario],
                    PSY.get_component(
                        typeof(PSY_project_PRAS),
                        sys_PRAS[scenario],
                        PSY.get_name(PSY_project_PRAS),
                    ),
                    product,
                )
            end
        ...
        for y in 1:simulation_years
            availability_df_rt =
                slice_year(availability_rt_by_scenario[pcm_scenario], y, simulation_years)
            availability_df =
                slice_year(availability_by_scenario[pcm_scenario], y, simulation_years)
            ...
            if y >= iteration_year
                add_device_forecast!(sys_MDs[y], sys_UCs[y], sys_EDs[y],
                    PSY.get_component(typeof(PSY_project_MD), sys_MDs[y],
                        PSY.get_name(PSY_project_MD)),
                    PSY.get_component(typeof(PSY_project_UC),
                        sys_UCs[y], PSY.get_name(PSY_project_UC)),
                    PSY.get_component(typeof(PSY_project_ED),
                        sys_EDs[y], PSY.get_name(PSY_project_ED)), availability_raw, availability_raw_rt,
                    da_resolution, rt_resolution)

                if project_type == "NU_ST" || project_type == "RE_CT" ||
                   typeof(PSY_project_UC) == PSY.RenewableDispatch ||
                   typeof(PSY_project_UC) == PSY.HydroTurbine ||
                   typeof(PSY_project_UC) == PSY.HydroDispatch
                    # convert_to_thermal_clean_energy!(PSY_project_UC, sys_UCs[y])
                    # convert_to_thermal_clean_energy!(PSY_project_ED, sys_EDs[y])
                    add_clean_energy_contribution!(
                        sys_MDs[y],
                        PSY.get_component(
                            typeof(PSY_project_MD),
                            sys_MDs[y],
        ...
        for scenario in keys(sys_PRAS)
            ...
            elseif in("$(project_type)_$(project_zone)", names(availability_df_rt))
                availability_raw_rt =
                    availability_df_rt[:, Symbol("$(project_type)_$(project_zone)")]
            end
            ...
            if project_type == "NU_ST" || project_type == "RE_CT" ||
               typeof(PSY_project_PRAS) == PSY.RenewableDispatch ||
               typeof(PSY_project_PRAS) == PSY.HydroTurbine ||
               typeof(PSY_project_PRAS) == PSY.HydroDispatch
                if project_type == "RE_CT"
                    convert_to_thermal_fast_start!(
                        PSY.get_component(
                            typeof(PSY_project_PRAS),
