...
function construct_net_load_forecast_error_distribution(simulation_dir::String,
    ...
    filter!(
        row -> in(row["Month"], months) && in(row["Period"], rt_periods),
        load_n_vg_df_rt,
    )
    ...
    if zonal
        ...
        for zone in zones
            zonal_gens[zone] =
                filter(p -> get_zone(get_tech(p)) == zone, renewable_generators)
            ...
            load_n_vg_df_rt[:, "Net Load_$(zone)"] =
                load_n_vg_df_rt[:, "load_$(zone)"] - total_gen_rt
            load_n_vg_df_extrap[:, "Net Load_$(zone)"] =
                load_n_vg_df_extrap[:, "load_$(zone)"] - total_gen_extrap

            net_load_forecast_error[:, zone] =
                load_n_vg_df_rt[:, "Net Load_$(zone)"] -
                load_n_vg_df_extrap[:, "Net Load_$(zone)"]
        ...
        net_load_forecast_error[:, "Total"] =
            load_n_vg_df_rt[:, "Net Load"] - load_n_vg_df_extrap[:, "Net Load"]
...
function construct_conv_unavailabilities(simulation_dir::String,
    ...
    if ordc_unavailability_method == "Convolution"
        ...
        zones =
            chop.(filter(n -> occursin("load", n), names(load_n_vg_df)); head = 5, tail = 0)
...
function calculate_min_reserve_req(simulation_dir::String,
    ...
    if zonal
        ...
        for zone in zones
            largest_gen = maximum(
                get_maxcap.(filter(p -> get_zone(get_tech(p)) == zone, thermal_generators)),
            )
