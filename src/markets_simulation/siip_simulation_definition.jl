...
function update_realized_reserve_perc!(device::PSY.Device,
    ...
    if service_name in rt_products
        reserve_provision = results_ed[Symbol("$(service_name)__VariableReserve_ReserveUp")][:, Symbol(get_name(device))]
    ...
    elseif service_name in only_da_products
        reserve_provision = results_uc[Symbol("$(service_name)__VariableReserve_ReserveUp")][:, Symbol(get_name(device))]
        reserve_perc_value = reserve_provision / get_device_size(device)
        reserve_perc[get_name(device)][service_name][1, :] = reserve_perc_value
...
function update_realized_reserve_perc!(device::PSY.Device,
    ...
    if service_name in rt_products
        reserve_provision = results_ed[Symbol("$(service_name)__ReserveDemandCurve_ReserveUp")][:, Symbol(get_name(device))]
        reserve_perc_value = reserve_provision / get_device_size(device)
        reserve_perc[get_name(device)][service_name][1, :] = reserve_perc_value

    elseif service_name in only_da_products
        reserve_provision = results_uc[Symbol("$(service_name)__ReserveDemandCurve_ReserveUp")][:, Symbol(get_name(device))]
        reserve_perc_value = reserve_provision / get_device_size(device)
        reserve_perc[get_name(device)][service_name][1, :] = reserve_perc_value
...
function update_realized_reserve_perc!(device::PSY.Device,
    ...
    if service_name in rt_products
        reserve_provision = results_ed[Symbol("$(service_name)__VariableReserve_ReserveDown")][:, Symbol(get_name(device))]
        reserve_perc_value = reserve_provision / get_device_size(device)
        reserve_perc[get_name(device)][service_name][1, :] = reserve_perc_value

    elseif service_name in only_da_products
        reserve_provision = results_uc[Symbol("$(service_name)__VariableReserve_ReserveDown")][:, Symbol(get_name(device))]
        reserve_perc_value = reserve_provision / get_device_size(device)
        reserve_perc[get_name(device)][service_name][1, :] = reserve_perc_value
...
function update_realized_reserve_perc!(device::PSY.Device,
    ...
    if service_name == "Inertia"
        inertia_provision = results_ed[Symbol("$(service_name)__VariableReserve_ReserveSymmetric")][:, Symbol(get_name(device))]
...
function create_problem(template::PSI.OperationsProblemTemplate, sys::PSY.System, type::String, solver::JuMP.MOI.OptimizerWithAttributes, inertia_product)
    ...
    if in(PSY.VariableReserve{PSY.ReserveUp}, typeof.(services))
        push!(duals, Symbol("requirement__VariableReserve_ReserveUp"))
    end
    if in(PSY.ReserveDemandCurve{PSY.ReserveUp}, typeof.(services))
        push!(duals, Symbol("requirement__ReserveDemandCurve_ReserveUp"))
    end
    if in(PSY.VariableReserve{PSY.ReserveDown}, typeof.(services))
        push!(duals, Symbol("requirement__VariableReserve_ReserveDown"))
    end
    if in(PSY.VariableReserve{PSY.ReserveSymmetric}, typeof.(services))
        push!(duals, Symbol("requirement__VariableReserve_ReserveSymmetric"))
    end
...
function create_simulation( sys_UC::PSY.System,
    ...
    if isempty(inertia_product)
        feedforward_dict = Dict(
            ("ED", :devices, Symbol("ThermalStandard")) => PSI.SemiContinuousFF(
                binary_source_problem = PSI.ON,
                affected_variables = [PSI.ACTIVE_POWER],
            ),
            ("ED", :devices, Symbol("ThermalCleanEnergy")) => PSI.SemiContinuousFF(
                binary_source_problem = PSI.ON,
                affected_variables = [PSI.ACTIVE_POWER],
            ),
            ...
            ("ED", :devices, Symbol("HydroDispatch")) => PSIE.EnergyCommitmentFF(
                variable_source_problem = PSI.ACTIVE_POWER,
                affected_variables = [PSI.ACTIVE_POWER],
                affected_time_periods = 1,
            ),
            ("ED", :devices, Symbol("HydroEnergyReservoir")) => PSIE.EnergyCommitmentFF(
                variable_source_problem = PSI.ACTIVE_POWER,
                affected_variables = [PSI.ACTIVE_POWER],
                affected_time_periods = 1,
            ),
            ("ED", :devices, Symbol("RenewableDispatch")) => PSIE.EnergyCommitmentFF(
                variable_source_problem = PSI.ACTIVE_POWER,
                affected_variables = [PSI.ACTIVE_POWER],
                affected_time_periods = 1,
            ),
            ("ED", :devices, Symbol("GenericBattery")) => PSIE.EnergyTargetFF(
        ...
        feedforward_dict = Dict(
            ("ED", :devices, Symbol("ThermalStandard")) => PSIE.InertiaFF(
                binary_source_problem = PSI.ON,
                affected_variables = [PSI.ACTIVE_POWER],
            ...
            ("ED", :devices, Symbol("HydroEnergyReservoir")) => PSIE.EmisFF(
                binary_source_problem = PSI.ON,
                variable_source_problem = PSI.ACTIVE_POWER,
                affected_variables = [PSI.ACTIVE_POWER],
                affected_time_periods = 1,
            ...
            ("ED", :devices, Symbol("HydroDispatch")) => PSIE.EmisFF(
                binary_source_problem = PSI.ON,
                variable_source_problem = PSI.ACTIVE_POWER,
                affected_variables = [PSI.ACTIVE_POWER],
                affected_time_periods = 1,
    ...
    for service in get_system_services(sys_ED)
        ...
        if typeof(service) == PSY.VariableReserve{PSY.ReserveUp}
            reserve_price[name][1, :] = abs.(round.(dual_values_ed[Symbol("requirement__VariableReserve_ReserveUp")][:, Symbol("$(name)")], digits = 5)) / base_power
        ...
        elseif typeof(service) == PSY.ReserveDemandCurve{PSY.ReserveUp}
            reserve_price[name][1, :] = abs.(round.(dual_values_ed[Symbol("requirement__ReserveDemandCurve_ReserveUp")][:, Symbol("$(name)")], digits = 5)) / base_power
            replace!(reserve_price[name], NaN => 0.0)
            scale_voll(reserve_price[name], rt_resolution)
            #println(reserve_price[name])
        elseif typeof(service) == PSY.VariableReserve{PSY.ReserveDown}
            reserve_price[name][1, :] = abs.(round.(dual_values_ed[Symbol("requirement__VariableReserve_ReserveDown")][:, Symbol("$(name)")], digits = 5)) / base_power
            replace!(reserve_price[name], NaN => 0.0)
            scale_voll(reserve_price[name], rt_resolution)
            reserve_voll[name][1, :] = abs.(round.(result_variables_ed[Symbol("γ⁺__$(name)")][:, Symbol("γ⁺__$(name)")], digits = 5)) / base_power
            #println(reserve_price[name])
        elseif typeof(service) == PSY.VariableReserve{PSY.ReserveSymmetric}
            if name == "Inertia"
                inertia_price[1, :] = abs.(round.(dual_values_ed[Symbol("requirement__VariableReserve_ReserveSymmetric")][:, Symbol("$(name)")], digits = 5)) / base_power
    ...
    for service in get_system_services(sys_UC)
        ...
        if name in only_da_products
            if typeof(service) == PSY.VariableReserve{PSY.ReserveUp}
                reserve_price[name][1, :] = abs.(round.(dual_values_uc[Symbol("requirement__VariableReserve_ReserveUp")][:, Symbol("$(name)")], digits = 5)) / base_power
                replace!(reserve_price[name], NaN => 0.0)
            ...
            elseif typeof(service) == PSY.ReserveDemandCurve{PSY.ReserveUp}
                reserve_price[name][1, :] = abs.(round.(dual_values_uc[Symbol("requirement__ReserveDemandCurve_ReserveUp")][:, Symbol("$(name)")], digits = 5)) / base_power
                replace!(reserve_price[name], NaN => 0.0)
                scale_voll(reserve_price[name], da_resolution)
            elseif typeof(service) == PSY.VariableReserve{PSY.ReserveDown}
                reserve_price[name][1, :] = abs.(round.(dual_values_uc[Symbol("requirement__VariableReserve_ReserveDown")][:, Symbol("$(name)")], digits = 5)) / base_power
                replace!(reserve_price[name], NaN => 0.0)
                scale_voll(reserve_price[name], da_resolution)
                reserve_voll[name][1, :] = abs.(round.(result_variables_uc[Symbol("γ⁺__$(name)")][:, Symbol("γ⁺__$(name)")], digits = 5)) / base_power
            elseif typeof(service) == PSY.VariableReserve{PSY.ReserveSymmetric}
                if name == "Inertia"
                    inertia_price[1, :] = abs.(round.(dual_values_uc[Symbol("requirement__VariableReserve_ReserveSymmetric")][:, Symbol("$(name)")], digits = 5)) / base_power
                    replace!(inertia_price, NaN => 0.0)
