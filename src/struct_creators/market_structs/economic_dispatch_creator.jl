...
function create_economic_dispatch_problem(simulation::AgentSimulation,
    ...
    if isnothing(sys_UC)
        ...
        # Gather markets data-------------------------------------------------------------------------------
        reserve_up_demand_data = read_data(
            joinpath(
                timeseries_data_dir,
                pcm_scenario,
                "sim_year_$(iteration_year)",
        ...
        reserve_down_demand_data = read_data(
            joinpath(
                timeseries_data_dir,
                pcm_scenario,
                "sim_year_$(iteration_year)",
        ...
        energy_mkt_params =
            read_data(joinpath(simulation_dir, "markets_data", "energy_mkt_param.csv"))
        price_cap_energy = AxisArrays.AxisArray(energy_mkt_params.price_cap * 1.0, zones)

        reserve_up_mkt_params =
            read_data(joinpath(simulation_dir, "markets_data", "reserve_up_mkt_param.csv"))
        ...
        reserve_down_mkt_params = read_data(
            joinpath(simulation_dir, "markets_data", "reserve_down_mkt_param.csv"),
        )
        price_cap_reservedown =
            AxisArrays.AxisArray(reserve_down_mkt_params.price_cap * 1.0, zones)
        ...
        for (zone_num, zone) in enumerate(zones)
            ...
            if reserve_up_mkt_params.ORDC[idx]
                ...
                reserve_up_break_points[zone] =
                    [0.0, 1.0, 1.0, 10.0] * reserve_up_market_bool
                reserve_up_price_points[zone] =
                    [
                        reserve_up_mkt_params.price_cap[idx],
                        reserve_up_mkt_params.price_cap[idx],
                        0.0,
                        0.0,
                    ] * reserve_up_market_bool
            ...
            for h in 1:num_hours
                zonal_reserve_up[zone, h] =
                    reserve_up_demand_data[:, Symbol(zone_num)][h] * reserve_up_market_bool
                zonal_reserve_down[zone, h] =
                    reserve_down_demand_data[:, Symbol(zone_num)][h] *
                    reserve_down_market_bool
            end
        end
        ...
        capacity_mkt_param_file =
            joinpath(simulation_dir, "markets_data", "capacity_mkt_param.csv")
        peak_load = get_peak_load(simulation)[pcm_scenario][iteration_year]
        #capacity_annual_increment = load_growth

        REC_mkt_params =
            read_data(joinpath(simulation_dir, "markets_data", "REC_mkt_param.csv"))
        ...
        for p in 1:num_invperiods
            ...
            energy_markets[p] = EnergyMarket(
                AxisArrays.AxisArray(
                    zonal_load .* ((1 .+ energy_annual_increment) .^ p),
                    zones,
                    (1:num_hours),
                ),
                price_cap_energy)
            reserve_up_markets[p] = ReserveUpMarket(reserve_up_break_points,
                ...
                AxisArrays.AxisArray(
                    zonal_reserve_up .* ((1 .+ reserveup_annual_increment) .^ p),
                    zones,
                    (1:num_hours),
                ))
            reserve_down_markets[p] = ReserveDownMarket(
                AxisArrays.AxisArray(
                    zonal_reserve_down .* ((1 .+ reservedown_annual_increment) .^ p),
                    zones,
                    (1:num_hours),
                ),
                price_cap_reservedown)
            rec_markets[p] =
                RECMarket(min(rec_req + rec_annual_increment * (p + iteration_year - 1), 1),
                    price_cap_rec)
        end
