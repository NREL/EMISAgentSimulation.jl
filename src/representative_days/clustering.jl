...
function cluster(period_labels::AbstractVector,
    ...
    period_clusters = sort(
        Dict(
            periods[i] => medoid_periods[result.assignments[i]] for i in 1:length(periods)
        ),
    )
...
function prune_extra_rows!(df, interval)
    last_row =
        findlast(x -> x == Int(floor(nrow(df) / interval)), df[:, "Representative_Period"])
...
function find_representative_periods(simulation_dir::String,
    ...
    representative_periods_file = joinpath(
        base_dir,
        "rep_periods_$(scenario)_y$(sim_year)_$(interval)h_$(n_clusters)c.csv",
    )
    cluster_indices_file = joinpath(
        base_dir,
        "cluster_indices_$(scenario)_y$(sim_year)_$(interval)h_$(n_clusters)c.csv",
    )
    ...
    if isfile(representative_periods_file) && isfile(cluster_indices_file)
        ...
        clusters = sort(
            Dict(
                representative_periods_data[i, "Period"] =>
                    representative_periods_data[i, "Weight"] for
                i in 1:DataFrames.nrow(representative_periods_data)
            ),
        )
        ...
        allperiods = sort(
            Dict(
                cluster_indices_data[i, "Period"] =>
                    cluster_indices_data[i, "Rep_Period"] for
                i in 1:DataFrames.nrow(cluster_indices_data)
            ),
        )

    else
        net_load_data = read_data(
            joinpath(
                timeseries_data_dir,
                scenario,
                "sim_year_$(sim_year)",
        ...
        net_load_data[!, "Representative_Period"] =
            add_representative_period.(net_load_data[:, "Period_Number"], interval)
        ...
        representative_periods_data =
            DataFrames.DataFrame(; Period = Int64[], Weight = Int64[])
        cluster_indices_data =
            DataFrames.DataFrame(; Period = Int64[], Rep_Period = Int64[])
