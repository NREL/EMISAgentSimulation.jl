...
function make_thermal_tech()
    return ThermalTech(
        ...
        PSY.ThermalGenerationCost(
            variable  = PSY.CostCurve(PSY.PiecewiseLinearData([(0.0, 0.0), (400.0, 1200.0)])),
            fixed     = 5000.0,
...
function make_renewable_tech()
    return RenewableTech(
        ...
        PSY.RenewableGenerationCost(
            ...
            curtailment_cost = PSY.CostCurve(PSY.PiecewiseLinearData([(0.0, 0.0), (100.0, 0.0)])),
            fixed            = 0.0,
        ),
        "BusB",
        "ZoneB",
...
function make_market_prices()
    ep = Dict(s => AxisArrays.AxisArray(
            rand(N_YRS, N_HRS, 2),
            AxisArrays.Axis{:year}(1:N_YRS),
            AxisArrays.Axis{:hour}(1:N_HRS),
...
function make_investor(name::String)
    projects = Project{<:BuildPhase}[
        ThermalGenEMIS{Existing}("CC_plant", make_thermal_tech(),    1, 1, 30, 30, make_products(), make_finance()),
        RenewableGenEMIS{Option}("wind_opt",  make_renewable_tech(), 3, 5, 99, 99, make_products(), make_finance()),
        HydroGenEMIS{Planned}("hydro_pl",    make_hydro_tech(),      2, 4, 50, 50, make_products(), make_finance()),
        BatteryEMIS{Queue}("batt_q",          make_battery_tech(),    2, 3, 25, 25, make_products(), make_finance()),
    ]
    return Investor(
        name,
        "/data/$name",
...
function make_simulation()
    ...
    rep_periods = Dict(s => Dict(y => Dict(h => h for h in 1:N_HRS) for y in 1:N_YRS) for s in SCENS)
...
function compare_forecast(orig, loaded, path::String)
    ...
    for (i, (so, sl)) in enumerate(zip(orig.scenario_data, loaded.scenario_data))
        ...
        @test vals_equal(so.parameter_multipliers, sl.parameter_multipliers, "$path.scenario[$i].multipliers")
...
function compare_investor(orig::Investor, loaded::Investor, path::String)
    ...
    @test vals_equal(orig.preference_multiplier_range, loaded.preference_multiplier_range, "$path.preference_multiplier_range")
    ...
    @test vals_equal(orig.portfolio_preference_multipliers, loaded.portfolio_preference_multipliers, "$path.portfolio_pref_mult")
